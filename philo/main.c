...
/*   Updated: 2023/10/18 20:36:41 by minjeon2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	take_fork(t_data *data)
{
	if (data -> id % 2 == 1)
	{
		if (!take_a_left_fork(data, data->fork, \
		data->printf_mutex, data -> philo_info))
			return (0);
		if (!take_a_right_fork(data, data->fork, \
		data->printf_mutex, data -> philo_info))
			return (0);
	}
	else
	...
	if (!eat(data, data->philo_info, data->fork, data->printf_mutex))
		return (0);
	return (1);
}
...
void	start_philos_threads(t_philo_info *philos_info, pthread_mutex_t *fork)
{
	t_data				*data;
	struct timeval		time;
	pthread_t			*threads;
...
int	main(int argc, char **argv)
{
	...
	start_philos_threads(philo_info, fork);
	free(fork);
