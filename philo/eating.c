...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/20 19:25:34 by minjeon2          #+#    #+#             */
/*   Updated: 2023/11/20 20:00:13 by minjeon2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

void	renew_last_eating_time(t_data *data, struct timeval time)
{
	pthread_mutex_lock(data -> philo_info -> eating_mutex);
	data -> philos[data -> id].last_eating = get_time_in_milliseconds(&time);
	pthread_mutex_unlock(data -> philo_info -> eating_mutex);
}
...
pthread_mutex_t *fork_mutex, pthread_mutex_t *printf_mutex)
{
	...
	gettimeofday(&time, 0);
	renew_last_eating_time(data, time);
	pthread_mutex_lock(philo_info -> termination_philo_mutex);
	if (philo_info -> died_philo || philo_info -> all_full)
	{
		pthread_mutex_unlock(&fork_mutex[data->philos[data->id].left_fork]);
